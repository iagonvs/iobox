...
<div class="box">
    ...
    {{-- <div class="box-body table-responsive">
        ...
        <div class="col-lg-24">
        <a href="{{route('cadastrar_empresa')}}">
...
<table id="example1" class="table table-bordered table-striped">
    ...
    <tbody>
      <tr>
        <th scope="row">{{$listar->idEmpresa}}</th>
        <td>{{$listar->Empresa}}</td>
       
        <td><a href="{{route('cadastrar_empresa.edit', $listar->idEmpresa)}}">Editar</a></td>
      
        <form onclick="return confirm('Deseja excluir?');" action="{{route('cadastrar_empresa.destroy', $listar->idEmpresa)}}" method="POST">
          @csrf
          @method('DELETE')
